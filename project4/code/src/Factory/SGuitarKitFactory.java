...
public class SGuitarKitFactory implements GuitarKitFactory{
    ...
    public knobSet pickKnobSet(){
        ...
        knobSet k1=new knobSet("knobSet",2.00,true,0,5);
        knobSet k2=new knobSet("knobSet",4.00,true,0,5);
        knobSet k3=new knobSet("knobSet",6.00,true,0,5);
        knobSetOpt.add(k1);
        knobSetOpt.add(k2);
        knobSetOpt.add(k3);
        while(response!=0 && response!=1 && response!=2){
    ...
    public neck pickNeck(){
        ...
        neck n1=new neck("neck",3.00,true,0,5);
        neck n2=new neck("neck",6.00,true,0,5);
        neck n3=new neck("neck",9.00,true,0,5);
        nckOpt.add(n1);
        nckOpt.add(n2);
        nckOpt.add(n3);
        while(response!=0 && response!=1 && response!=2){
    ...
    public pickGuard pickpG(){
        ...
        pickGuard pg1=new pickGuard("pickGuard",4.00,true,0,5);
        pickGuard pg2=new pickGuard("pickGuard",8.00,true,0,5);
        pickGuard pg3=new pickGuard("pickGuard",12.00,true,0,5);
        pGOpt.add(pg1);
        pGOpt.add(pg2);
        pGOpt.add(pg3);
        while(response!=0 && response!=1 && response!=2){
    ...
    public pickUps pickpU(){
        ...
        pickUps pu2=new pickUps("pickUps",10.00,true,0,5);
        pickUps pu3=new pickUps("pickUps",15.00,true,0,5);
        pUOpt.add(pu1);
        pUOpt.add(pu2);
        pUOpt.add(pu3);
        while(response!=0 && response!=1 && response!=2){
